...
class ThemeSelectorLogic:
    ...
    def get_stylesheet(theme_name: str | None = None) -> str:
        ...
        QFrame#leftPanel, QFrame#rightPanel {{ background-color: {t['bg_panel']}; border: 1px solid {panel_border}; border-radius: 10px; }}
        QFrame#narrowPanel {{ background: {narrow_bg}; border: 1px solid {narrow_border}; border-radius: 10px; }}
        QLabel {{ color: {t['text']}; }}
        QLabel[secondary='true'] {{ color: {t['text_muted']}; }}
        QLabel#dropArea {{ background: {drop_bg}; color: {drop_color}; font-size: 18px; border: {drop_border}; border-radius: {drop_radius}; }}
        ...
        QPushButton {{ background-color: {t['button_bg']}; color: {t['text']}; border: 1px solid {t['border']}; border-radius: 6px; padding: 6px 10px; }}
        ...
        QPushButton#iconButton {{ background-color: transparent; border: none; color: {icon_color}; font-size: 20px; border-radius: 6px; }}

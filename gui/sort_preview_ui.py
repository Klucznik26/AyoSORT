...
class PreviewUI(QFrame):
    def __init__(self, parent=None):
        ...
        self.image_fan = PreviewFanWidget()
        self.image_fan.hide()
        fan_policy = QSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Expanding)
        fan_policy.setRetainSizeWhenHidden(True)
        self.image_fan.setSizePolicy(fan_policy)
        ...
        QTimer.singleShot(0, self._load_logo)

    def _create_glass(self, parent_layout, stretch):
        frame = QFrame()
        frame.setStyleSheet(
            "QFrame { background-color: rgba(150, 150, 150, 0.1); border-radius: 12px; border: 1px solid rgba(150, 150, 150, 0.15); }"
        )
